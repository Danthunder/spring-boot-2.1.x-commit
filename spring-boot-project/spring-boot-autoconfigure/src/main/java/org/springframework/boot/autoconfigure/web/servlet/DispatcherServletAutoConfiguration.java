 ...
 * @since 2.0.0
 *
 * SpringBootServletInitializer 是SpringBoot用于打war包时留给Web容器初始化应用的钩子。根据
 * AutoConfigureAfter 注解，该类要在 ServletWebServerFactoryAutoConfiguration 之后再执行
 *
 * AutoConfigureOrder 注解中的HIGHEST_PRECEDENCE 是最高优先级
 *
 * 里面嵌套了四个内部类，有两个分别注册 DispatcherServlet 和 DispatcherServletRegistrationBean
 */
@AutoConfigureOrder(Ordered.HIGHEST_PRECEDENCE)
@Configuration
@ConditionalOnWebApplication(type = Type.SERVLET) // SERVLET环境才生效
...
public class DispatcherServletAutoConfiguration {
	...
	@Configuration
	// 当 DefaultDispatcherServletCondition（matches方法）的条件成立，才给容器添加组件
	@Conditional(DefaultDispatcherServletCondition.class)
	@ConditionalOnClass(ServletRegistration.class)
	// 启动配置文件与HttpProperties和WebMvcProperties的映射
	...
	protected static class DispatcherServletConfiguration {
		...
		// 构造 DispatcherServlet，将默认的配置设置到 DispatcherServlet 中
		// 这些属性来自 HttpProperties 和 WebMvcProperties
	...
	protected static class DispatcherServletRegistrationConfiguration {
		...
		// 辅助注册 DispatcherServlet 的 DispatcherServletRegistrationBean
