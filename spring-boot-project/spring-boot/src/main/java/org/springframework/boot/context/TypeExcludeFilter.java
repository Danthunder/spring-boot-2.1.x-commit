 ...
 * @since 1.4.0
 *
 * spring boot的一种扩展机制，能够让程序员向spring ioc容器中注册一些自定义的组件过滤器，
 * 以便在包扫描的过程中过滤它们。核心逻辑为match方法，实现了过滤的判断逻辑
 */
public class TypeExcludeFilter implements TypeFilter, BeanFactoryAware {
	...
	public boolean match(MetadataReader metadataReader, MetadataReaderFactory metadataReaderFactory)
			throws IOException {
		// 从beanFactory（spring容器的bean工厂）中获取所有类型为 TypeExcludeFilter
		// 的组件delegates, 去执行自定义的过滤方法，即match方法。
